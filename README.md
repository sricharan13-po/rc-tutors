...
| `DATABASE_URL` | For production | Supabase/Postgres connection string. Without it, data lives in memory / `db.json` and resets on restart. |
...
1. **Database** — create a free [Supabase](https://supabase.com) project (no credit card needed), copy the Postgres connection string from Project Settings → Database → Connection string (URI), and set it as `DATABASE_URL`. The app creates its tables automatically on first boot. Without this, a server restart wipes all accounts, payments, and enrolments.
